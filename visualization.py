...
def preprocess_cell(img, contour):
    ...
    for c in range(3):
        dest[..., c] = alpha * rgb[..., c] + (1.0 - alpha) * dest[..., c]
...
def analyze_single_image(img_id, model, thresholds, transform, progress_callback=None):
    ...
    if not f_img:
        return None, f"Image {img_id} not found.", None, None
    if not f_json:
        return None, f"JSON {img_id} not found.", None, None
    try:
...
class ResultWindow:
    def __init__(self, parent, results_list):
        ...
        tk.Checkbutton(
            group_layers, text="Segmentation Mask",
            var=self.show_segmentation, font=("Segoe UI", 10), fg="#388e3c",
            command=self.refresh_image
        ).pack(anchor="w", pady=2)
        tk.Checkbutton(
            group_layers, text="Key Diagnostic Cells",
            var=self.show_key_cells, font=("Segoe UI", 10, "bold"), fg="#d32f2f",
            command=self.refresh_image
        ).pack(anchor="w", pady=2)
        tk.Label(
            group_layers, text="(Hover over red cells to see details)",
            font=("Segoe UI", 9, "italic"), fg="gray"
        ).pack(anchor="w", pady=2)
        btn_save = tk.Button(
            self.frame_ctrl, text="Export Current Image", font=("Segoe UI", 11),
    ...
    def refresh_image(self):
        ...
        cell_list = self.analysis_data['data']
...
class CancerApp:
    def __init__(self, root):
        ...
        tk.Label(
            root, text="THYROID CANCER DIAGNOSIS SYSTEM",
            font=("Segoe UI", 18, "bold"), pady=20, fg="#2c3e50"
        ).pack()
        ...
        self.listbox = tk.Listbox(
            frame_list, font=("Consolas", 11), height=12, selectmode=tk.EXTENDED,
    ...
    def process_thread(self, img_ids):
        ...
        try:
            ...
            for idx, img_id in enumerate(img_ids):
                ...
                img_clean, msg, vis_data, export_data = analyze_single_image(
                    img_id, model, thresholds, tf, img_progress_cb
                )
                if img_clean is not None:
                    json_path = os.path.join(JSON_OUT_DIR, f"{img_id}_detailed_results.json")
                    with open(json_path, 'w') as f:
                        json.dump({img_id: export_data}, f, indent=4)
